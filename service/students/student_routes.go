...
// func to handler request for delete endpoint
func (h *HandleRequest) Delete_Bp(w http.ResponseWriter, r *http.Request) {
	...
	uuid_user, err := uuid.Parse(id)
	if err != nil {
		logger.Log.Error("Invalid type",
			zap.String("request_id", requestID),
			zap.String("client_ip", r.RemoteAddr),
		)
		utils.ResponseError(w, http.StatusBadRequest, "Failed to convert into an uuid type!", err.Error())
		return
	}
	if uuid_user == uuid.Nil {
		utils.ResponseError(w, http.StatusBadRequest, "Failed go get the uuid, because the result for convert is nill", false)
		return
	}

	//execute the query for store
	ctx, cancle := context.WithTimeout(r.Context(), time.Second*10)
	defer cancle()
	if err := h.db.DeleteStudentsData(uuid_user, ctx); err != nil {
		logger.Log.Error("request_id",
			zap.String("request_id", requestID),
			zap.String("client_ip", r.RemoteAddr),
		)
		utils.ResponseError(w, http.StatusBadRequest, "Failed to delete the students data as a admin!", err.Error())
		return
	}

	//final return
	utils.ResponseSuccess(w, http.StatusOK, "Deleting students data as a admin has been successfully!", true)
