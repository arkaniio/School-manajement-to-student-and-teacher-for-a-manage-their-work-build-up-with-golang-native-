...
func (h *HanlderAbsensi) CreateNewAbsensi_Bp(w http.ResponseWriter, r *http.Request) {
	...
	if role != "guru" && role != "admin" && role != "siswa" {
		logger.Log.Warn("Access denied invalid role",
			zap.String("role", role))
		utils.ResponseError(w, http.StatusForbidden, "Forbidden: invalid role", nil)
		return
	}
	...
	// parse student_id from payload
	var studentId uuid.UUID
	if payloads.StudentId != "" {
		studentId, err = uuid.Parse(payloads.StudentId)
		if err != nil {
			utils.ResponseError(w, http.StatusBadRequest, "Invalid student_id format!", err.Error())
			return
		}
	}
	if studentId == uuid.Nil {
		utils.ResponseError(w, http.StatusBadRequest, "student_id is required!", false)
		return
	}
	...
	if err := h.db.CreateNewAbsensi(ctx, absensi); err != nil {
		logger.Log.Error("Failed to create absensi",
			zap.Error(err),
			zap.String("request_id", request_id))
		utils.ResponseError(w, http.StatusInternalServerError, "Failed to create absensi record", err.Error())
		return
	}

	utils.ResponseSuccess(w, http.StatusCreated, "Absensi created successfully", absensi)
}
