...
func (s *StoreAbsensi) CreateNewAbsensi(ctx context.Context, payloads *types.Absensi) error {
	...
	//execute the INSERT using ExecContext (avoids RETURNING * column-order scan issues)
	query_exec := `
		INSERT INTO absensis (id, name_lengkap, kelas, jurusan, hari, tanggal, status, keterangan, created_at, updated_at, keterangan_tidak_hadir, keterangan_dispen, file_dispen, student_id)
