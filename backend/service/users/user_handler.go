...
func (h *HandleRequest) Login_Bp(w http.ResponseWriter, r *http.Request) {
	...
	response_users := map[string]interface{}{
		"id":            users.Id,
		"email":         users.Email,
	...
	if err == nil {
		...
		if users_profile_image.Profile_Image != "" {
			...
			if _, err := os.Stat(path_old); !os.IsNotExist(err) {
				// Old file exists — remove it (ignore error, best-effort cleanup)
				if err := os.Remove(path_old); err != nil {
					logger.Log.Warn("Failed to remove old profile image",
						zap.String("request_id", requestID),
						zap.String("path", path_old),
						zap.Error(err),
					)
					// Do NOT return — we still save the new image even if cleanup fails
				}
			}
		}
...
func (h *HandleRequest) Image_Bp(w http.ResponseWriter, r *http.Request) {
	...
	path_name := filepath.Join("uploadsUser", filename)

	//open the file manually (avoids http.ServeFile redirect side-effects with gorilla/mux)
	f, err := os.Open(path_name)
	if err != nil {
		if os.IsNotExist(err) {
			logger.Log.Error("Profile image file not found",
				zap.String("request_id", requestID),
				zap.String("path", path_name),
			)
			utils.ResponseError(w, http.StatusNotFound, "Image file not found!", false)
			return
		}
		logger.Log.Error("Failed to open profile image file",
			zap.String("request_id", requestID),
			zap.String("path", path_name),
		)
		utils.ResponseError(w, http.StatusInternalServerError, "Failed to open image!", err.Error())
		return
	}
	defer f.Close()
	...
	stat, err := f.Stat()
	if err != nil {
		utils.ResponseError(w, http.StatusInternalServerError, "Failed to read image info!", err.Error())
		return
	}

	//set CORS header so browser can access the image
