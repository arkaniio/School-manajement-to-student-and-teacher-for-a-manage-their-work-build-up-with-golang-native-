...
func (h *HanlderStudentsGrade) GetGradesByStudentId_Bp(w http.ResponseWriter, r *http.Request) {

	//get the request id from middleware
	request_id := middleware.GetRequestID(r)
	if request_id == "" {
		logger.Log.Info("Failed to get request id for student grades!",
			zap.String("client_ip", r.RemoteAddr),
			zap.String("path", r.URL.Path),
		)
		utils.ResponseError(w, http.StatusBadRequest, "Failed to get request id!", false)
	...
	role, err := middleware.GetRoleMiddleware(w, r)
	if err != nil {
		logger.Log.Error("Failed to get role for student grades!",
			zap.String("request_id", request_id),
		)
		utils.ResponseError(w, http.StatusBadRequest, "Failed to get role!", err.Error())
		return
	}
	// Allow both guru and siswa (siswa can only see their own via the ID param, and we trust the student_id from URL for now, but in a real app we'd cross-check with token ID)
	if role != "guru" && role != "siswa" {
		utils.ResponseError(w, http.StatusBadRequest, "Invalid role!", false)
		return
	}
	...
	if err != nil {
		logger.Log.Error("Invalid student ID for grades!",
			zap.String("request_id", request_id),
			zap.String("student_id", studentIdStr),
		)
		utils.ResponseError(w, http.StatusBadRequest, "Invalid student ID!", err.Error())
		return
	}
	...
	grades, err := h.repo.GetGradesByStudentId(ctx, studentId)
	if err != nil {
		logger.Log.Error("Failed to get grades by student id!",
			zap.String("request_id", request_id),
