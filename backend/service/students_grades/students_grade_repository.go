...
func (s *StoreStudentsGrade) GetGradesByStudentId(ctx context.Context, student_id uuid.UUID) ([]types.StudentsGradesWithTaskResponse, error) {
	query := `
		SELECT sg.id, sg.task_id, sg.tanggal, sg.keterangan, sg.grades, sg.created_at, sg.updated_at,
		       t.name_task, t.mapel_task
		FROM students_grades sg
	...
	var grades []types.StudentsGradesWithTaskResponse
	err := s.db.SelectContext(ctx, &grades, query, student_id)
	if err != nil {
		return nil, errors.New("Failed to get students grades by student id!")
	}
