...
func (h *HandleTaskRequest) Create_TaskBp(w http.ResponseWriter, r *http.Request) {
	...
	// get user id from token (used as student_id — safer than trusting form value)
	token_user_id, err := middleware.GetIdMiddleware(w, r)
	if err != nil {
		utils.ResponseError(w, http.StatusBadRequest, "Failed to get user id from token!", err.Error())
		return
	}
	...
	if name_task == "" {
		utils.ResponseError(w, http.StatusBadRequest, "Failed to detect the form file in request: name_task is required", false)
		return
	}
	//use student_id from form if provided, otherwise use the token user id
	var student_id_fix uuid.UUID
	if student_id != "" {
		var parseErr error
		student_id_fix, parseErr = uuid.Parse(student_id)
		if parseErr != nil {
			utils.ResponseError(w, http.StatusBadRequest, "Failed to parsing the student id into an uuid type!", parseErr.Error())
			return
		}
	} else {
		student_id_fix = token_user_id
	}
	...
	//reset seek after reading buff for content detection
	file_task.Seek(0, 0)

	//check the content type for the file task (allow images and PDF)
	check_content_type := http.DetectContentType(buff)
	if check_content_type != "image/jpg" && check_content_type != "image/png" && check_content_type != "image/jpeg" && check_content_type != "application/pdf" {
		utils.ResponseError(w, http.StatusBadRequest, "Invalid content type for the file task! Allowed: jpg, png, jpeg, pdf", false)
		return
	}

	//make the file name for the file task and folder that place to save the file task url
	file_name := uuid.New().String() + filepath.Ext(header.Filename)
	if file_name == "" {
...
func (h *HandleTaskRequest) ReadFile_Bp(w http.ResponseWriter, r *http.Request) {
	...
	//join the file name with a path folder (no leading slash — cross-platform)
	path_folder := "uploadsTask"
	file_path := filepath.Join(path_folder, filename)

	//open the file manually (avoids http.ServeFile redirect side-effects with gorilla/mux)
	f, err := os.Open(file_path)
	if err != nil {
		if os.IsNotExist(err) {
			logger.Log.Error("Task file not found",
				zap.String("request_id", request_id),
				zap.String("file_path", file_path),
			)
			utils.ResponseError(w, http.StatusNotFound, "File not found!", file_path)
			return
		}
		logger.Log.Error("Failed to open task file",
			zap.String("request_id", request_id),
			zap.String("file_path", file_path),
		)
		utils.ResponseError(w, http.StatusInternalServerError, "Failed to open file!", err.Error())
		return
	}
	defer f.Close()
	...
	stat, err := f.Stat()
	if err != nil {
		utils.ResponseError(w, http.StatusInternalServerError, "Failed to read file info!", err.Error())
		return
	}

	//set CORS header so browser can access the file directly
	w.Header().Set("Access-Control-Allow-Origin", "*")

	//serve file content (no redirect, no URL path manipulation)
...
// func to get tasks by student id (accessible by siswa)
func (h *HandleTaskRequest) GetTasksByStudentId_Bp(w http.ResponseWriter, r *http.Request) {
	...
	if role != "siswa" {
		utils.ResponseError(w, http.StatusUnauthorized, "Only siswa can access their own tasks!", false)
		return
	}

	//get the student_id from params url
	vars_id := mux.Vars(r)
	student_id := vars_id["student_id"]
	if student_id == "" {
		utils.ResponseError(w, http.StatusBadRequest, "Failed to get the student_id params!", false)
		return
	}
	...
	if err != nil {
		logger.Log.Error("Failed to parse student_id into uuid!",
			zap.String("request_id", request_id),
			zap.String("client_ip", r.RemoteAddr),
		)
		utils.ResponseError(w, http.StatusBadRequest, "Failed to parse student_id into uuid type!", err.Error())
		return
	}
	if student_id_fix == uuid.Nil {
		utils.ResponseError(w, http.StatusBadRequest, "Invalid student_id!", false)
		return
	}
	...
	ctx, cancle := context.WithTimeout(r.Context(), time.Second*10)
	defer cancle()
	tasks_data, err := h.repo.GetTasksByStudentId(student_id_fix, ctx)
	if err != nil {
		logger.Log.Error("Failed to get tasks by student id!",
			zap.String("request_id", request_id),
			zap.String("client_ip", r.RemoteAddr),
		)
		utils.ResponseError(w, http.StatusBadRequest, "Failed to get tasks for this student!", err.Error())
		return
	}

	//return final result
	utils.ResponseSuccess(w, http.StatusOK, "Get tasks by student id has been successfully!", tasks_data)
