...
// make func to get all tasks data
func (s *StoreTask) GetAllTaskIncludeStudents(ctx context.Context) ([]types.TaskWithStudents, error) {
...
// func to get all tasks by student id (for siswa/student role)
func (s *StoreTask) GetTasksByStudentId(student_id uuid.UUID, ctx context.Context) ([]types.Task, error) {

	//base query
	query := `
		SELECT id, name_task, file_task, date_task, student_id, created_at, updated_at, mapel_task
		FROM tasks
		WHERE student_id = $1
	...
	//execute the query
	var tasks []types.Task
	if err := s.db.SelectContext(ctx, &tasks, query, student_id); err != nil {
		if err == sql.ErrNoRows {
			return []types.Task{}, nil
		}
		return nil, errors.New("Failed to get the tasks by student id! " + err.Error())
	}
