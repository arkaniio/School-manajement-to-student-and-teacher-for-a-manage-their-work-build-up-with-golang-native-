...
func (h *HandleStudentsRequest) RegisterAsStudent_Bp(w http.ResponseWriter, r *http.Request) {
	...
	//get the user id from JWT middleware (to link student to user account)
	user_id, err := middleware.GetIdMiddleware(w, r)
	if err != nil {
		logger.Log.Error("Failed to get the user id from middleware!",
			zap.String("request_id", request_id),
			zap.String("client_ip", r.RemoteAddr),
		)
	...
	//check if this user already has a student record (enforce 1-per-user)
	ctxCheck, cancelCheck := context.WithTimeout(r.Context(), time.Second*10)
	defer cancelCheck()
	exists, err := h.repo.CheckStudentExistsByUserID(user_id, ctxCheck)
	if err != nil {
		logger.Log.Error("Failed to check student existence!",
			zap.String("request_id", request_id),
			zap.String("client_ip", r.RemoteAddr),
		)
		utils.ResponseError(w, http.StatusInternalServerError, "Failed to check student registration status!", err.Error())
		return
	}
	if exists {
		utils.ResponseError(w, http.StatusConflict, "You have already registered as a student! Only one registration per account is allowed.", false)
		return
	}

	//make decode for the payload
	...
	//make the validator for a payloads
	...
	if err := validate.Struct(&payloads); err != nil {
		...
		utils.ResponseError(w, http.StatusBadRequest, "Validation failed", errors)
		return
	}

	//parsing the payload into a students struct — link to current user
	...
	//make the time parsing for a response students
...
// func to handle the get students data — returns only the current user's own student record
func (h *HandleStudentsRequest) GetAllStudents_Bp(w http.ResponseWriter, r *http.Request) {
	...
	//get the student record belonging to this user
	ctx, cancle := context.WithTimeout(r.Context(), time.Second*10)
	defer cancle()
	student_data, err := h.repo.GetStudentByUserID(user_id, ctx)
	if err != nil {
		logger.Log.Error("Failed to get student data by user_id!",
			zap.String("request_id", request_id),
			zap.String("client_ip", r.RemoteAddr),
		)
		utils.ResponseError(w, http.StatusBadRequest, "Failed to get the student data!", err.Error())
		return
	}

	//if no student record found, return empty (not an error)
	if student_data == nil {
		utils.ResponseSuccess(w, http.StatusOK, "No student profile found for this user.", nil)
		return
	}

	//build response
	student_response := types.StudentResponse{
		Id:             student_data.Id,
		Full_name:      student_data.Full_Name,
		Kelas:          student_data.Kelas,
		Jurusan:        student_data.Jurusan,
		Absen:          student_data.Absen,
		StudentProfile: student_data.StudentProfile,
		Wali_Kelas:     student_data.Wali_Kelas,
		Created_at:     student_data.Created_at.Format("2006-01-02"),
		Updated_at:     student_data.Updated_at.Format("2006-01-02"),
		MapelStudents:  student_data.MapelStudents,
		User_Id:        student_data.User_Id,
	}

	//return final result
	utils.ResponseSuccess(w, http.StatusOK, "Get student data has been successfully!", student_response)
